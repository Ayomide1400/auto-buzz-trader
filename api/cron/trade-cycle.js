...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Persist sells immediately — a real sell already happened on the
    // account by this point, so its log entry must not depend on the buy
    // phase below also succeeding. Losing this would silently reproduce
    // the exact kind of gap that left one earlier trade (BTDR) unlogged.
    if (sellEvents.length) await appendTradeEvents(sellEvents)
    ...
    // Wrapped end-to-end: Alpaca rejects the ENTIRE batch snapshot request
    // if even one symbol in it is malformed (seen live: "401JK.X" from the
    // trending list poisoned the whole call) — without this, one bad
    // symbol crashes the cycle and nothing gets bought OR sold that run.
    const buyEvents = []
    try {
      ...
      // move actually is — with a limited number of slots, fill them with
      // the strongest qualifying movers first, not just the most talked about.
      ...
      errors.push(`buy phase: ${err.message}`)
    }

    if (buyEvents.length) await appendTradeEvents(buyEvents)
